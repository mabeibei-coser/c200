...
export async function downloadDouyinViaBrowser(rawUrl, opts = {}) {
  ...
  const launchOpts = {
    ...
    args: [
      "--no-sandbox", // Linux 以 root 跑必须；Windows 无害
  ...
  if (useExplicit) {
    launchOpts.executablePath = chromePath; // 指定了固定 Chrome 路径（含 CHROME_PATH 覆盖）
  } else if (process.platform === "win32") {
    launchOpts.channel = "chrome"; // 本地 Windows：用系统已装的 Chrome
  }
  // 否则 Linux/服务器：不指定，用 playwright 自带 chromium（需先 npx playwright install chromium）
  let browser;
  try {
    browser = await chromium.launch(launchOpts);
  } catch (e) {
    // 服务器/无浏览器环境下启动失败：给人话提示，别把 playwright 原始报错甩给用户
    console.error("[douyin] 浏览器启动失败:", e.message);
    throw new Error(
      "当前环境没有可用的浏览器，抖音链接无法在线解析。请改用「上传」：" +
        "在抖音 App 里把视频保存到本地，再上传文件解析（抖音抓取依赖真实浏览器，目前仅本地版可用）。"
    );
  }
