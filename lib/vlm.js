...
// 关键帧 → data URL（讯飞 vision 接受 base64 内联图，免得把本机帧暴露成公网 URL）
...
// 带重试：偶发瞬时 Connection error，连接类失败或 JSON 崩了都退避重试几次。
...
// 给一组镜头逐个描述（顺序执行，避免限流）。shots 取自 parseVideo 的结果，需带 keyframe 绝对路径。
