// C200 画面理解模块 —— 给一张镜头关键帧，用豆包 vision（火山方舟）输出结构化「镜头描述」。
// 用途：复刻参考视频时，先逐镜看懂"这镜在拍什么、主体是什么、景别/角度/光线"，
// 再喂给下游分镜脚本生成，并定位"可被替换的带货主体"。
// 复用 lib/image.js 同一套火山 ARK key（IMAGE_API_KEY），模型可用 VLM_MODEL 覆盖。

import OpenAI from "openai";
import fs from "node:fs";
import path from "node:path";

const DEFAULT_MODEL = "doubao-seed-1-6-250615"; // 已验证可用；快/省可切 doubao-seed-1-6-flash-250615
const ARK_URL = "https://ark.cn-beijing.volces.com/api/v3";

function arkClient() {
  return new OpenAI({
    apiKey: process.env.VLM_API_KEY || process.env.IMAGE_API_KEY || "no-key",
    baseURL: process.env.VLM_API_URL || process.env.IMAGE_API_URL || ARK_URL,
    maxRetries: 2,
  });
}
